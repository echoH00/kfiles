...
function kube_install() {
  ...
  if [ $proxy_flag == 'on' ]; then
    sudo curl -x ${http_proxy} \
      https://mirrors.aliyun.com/kubernetes/apt/doc/apt-key.gpg | sudo apt-key add -
  else
    sudo curl https://mirrors.aliyun.com/kubernetes/apt/doc/apt-key.gpg | sudo apt-key add -
  fi

  sudo add-apt-repository -y "deb https://mirrors.aliyun.com/kubernetes/apt/ kubernetes-xenial main"
